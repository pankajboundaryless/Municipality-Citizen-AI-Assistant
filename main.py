...
def _build_system_instruction(citizen: dict, prev_transcript: list | None = None) -> str:
    ...
    is_construction = "construction" in service.lower()
    requirements_block = (
        f"\n\n## Data Requirements for '{service}'\n\n"
        f"{requirements_section}\n\n"
        + (
            "For this Construction Permit request, collect ONLY: name, address, phone, email, "
            "and a brief description of the construction work. Ignore all other fields in the table — "
            "do not ask for drawings, permits, or project documents.\n"
            if is_construction else
            "Use the table above as your checklist. Work through the fields conversationally — "
            "do not read out the table literally. Ask one or two questions at a time.\n"
            "If they cannot provide a field, note it as missing and continue.\n"
        )
    ...
    return (
        ...
        "  - Read back a brief summary before submitting, but ask for confirmation ONCE only. "
        "If the citizen has already confirmed (e.g. 'yes', 'proceed', 'go ahead'), call "
        "submit_to_municipality immediately — do NOT ask again.\n\n"
        ...
        "2. For Identity Document (ID card) or Passport requests: on the FIRST question, mention "
        "that the citizen can upload a photo of their document to auto-fill the fields, "
        "OR answer verbally — whichever they prefer.\n"
        "3. For Construction Permit requests: collect ONLY personal data (name, address, phone, email) "
        "and a brief description of the construction work. Do NOT ask for a full document checklist — "
        "the citizen is making a general inquiry and any additional documents will be handled separately.\n"
        "4. Use the data requirements checklist below for all other services — collect fields "
        "conversationally, not as a rigid form. Ask one or two questions at a time.\n"
        "5. If the citizen says they do not have a field, note it as 'not provided' and move on.\n"
        "6. If documents were uploaded, acknowledge them by name and confirm their purpose.\n"
        "7. If the citizen shares their camera or screen, describe what you observe if relevant.\n"
        "8. When you have collected enough information, read back a brief summary and ask ONCE "
        "for confirmation to proceed. The moment the citizen confirms, call submit_to_municipality "
        "immediately — do not ask again or re-summarise.\n"
        "9. submit_to_municipality MUST always be called to finalise any request. "
        "Never tell the citizen their request has been registered or booked without having "
        "received a real response from submit_to_municipality first. "
        "Partial data is fine; Maestro handles validation and will return a status.\n"
        "10. Relay the Maestro response (reference number, status, or next steps) exactly as returned.\n\n"
